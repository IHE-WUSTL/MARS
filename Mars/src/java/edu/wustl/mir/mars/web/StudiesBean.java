...
@ManagedBean
@SessionScoped
public class StudiesBean extends BeanBase implements Serializable {
   ...
   static final long serialVersionUID = 1L;
    ...
    public StudiesBean() {
       sessionBean.newTaskBean("studiesBean");
   ...
   public Study getNewStudy() {return newStudy;}
   public void setNewStudy(Study newStudy) {this.newStudy = newStudy;}
   ...
   public void addNewStudy() {
      ...
      try {
         v.startValidations();
         v.NB("Study ID", newStudy.getId());
         newTransaction();
         Study s = (Study) session.createQuery("from study s where s.id = :id").
                 setString("id", newStudy.getId()).uniqueResult();
         if (s != null) v.error("Study Id", "already in use");
         if (v.isErrors()) {
            rollback();
            return;
         }
         ...
         syslog.warn("Error during add Study: " + e.getMessage());
         v.error("Error during add Study: ", e.getMessage());
      }
   }
  ...
  public boolean renderChgUsers()       { return currentChangeStep == 5; }
   ...
   public final void initializeChg() {
      ...
      try {
         ...
         List<Study> studies = (List<Study>) session.
                 createQuery("from study s order by s.id asc").list();
